...
def search(item, texto):
    logger.info("mitvspain.bricocine search")
    texto = texto.replace(" ", "+")
...
def peliculas(item, texto=""):
    logger.info("mitvspain.bricocine peliculas")
    itemlist = []
...
def fanart(item):
    ...
    logger.info("mitvspain.bricocine fanart")
    itemlist = []
    url = item.url
    data = get_page( item.url )
    data = re.sub(r"\n|\r|\t|\s{2}|\(.*?\)|\[.*?\]|&nbsp;","",data)
    ...
    return itemlist
def findvideos(item):
    logger.info("mitvspain.bricocine findvideos")
    ...
    if 'id="magnet"' in data:
        if 'id="file"' in data:
            bloque_capitulos= scrapertools.get_match(data,'<table class="table table-series">(.*?)<span class="block mtop clearfix">')
            ...
            for title_links, seasson, epi, calidad, title_torrent, url_torrent,title_magnet, url_magnet in matches:
                ...
                title_magnet = "["+"magnet"+"]"
                title_magnet = "[COLOR red]Opción[/COLOR]"+" "+ title_magnet.replace(title_magnet,"[COLOR crimson]"+title_magnet+"[/COLOR]")
            ...
            if checktorrent == "Descargar .torrent":
               torrent_bloque =scrapertools.get_match(data,'id="file".*?id="magnet".*?<span class="title">.*?<a id="file".*?a id="file".*?class="btn btn-primary".*?d="file"(.*?class="btn btn-primary".*?)</table>')
...
def episodios(item):
    logger.info("mitvspain.bricocine episodios")
    itemlist = []
    ...
    urltmdb_epi ="https://api.themoviedb.org/3/tv/"+id+"/season/"+item.extra.split("|")[0]+"/episode/"+item.extra.split("|")[2]+"/images?api_key=2e2160006592024ba87ccdf78c28f49f"
    ...
    return itemlist
def play(item):
    logger.info("mitvspain.bricocine play")
    itemlist = []
...
def findvideos_peli(item):
    logger.info("mitvspain.bricocine findvideos_peli")
    ...
    if 'id="magnet"' in data and not 'id="file"' in data:
        ...
        for title_links, title_magnet, url_magnet in matches:
            ...
            title_magnet = title_tag+title_links+"- "+title_magnet.replace(title_magnet,"[COLOR crimson]"+title_magnet+"[/COLOR]")
            url_magnet = base64.decodestring(url_magnet.split('&u=')[1][::-1])
            if "sinopsis.png" in item.extra:
                item.extra ="http://oi67.tinypic.com/28sxwrs.jpg"
            ###Se identifica si es una serie mal tipificada
...
def trailer(item):
    ...
    logger.info("mitvspain.bricocine trailer")
...
def info(item):
    ...
    logger.info("mitvspain.bricocine info")
    url=item.url
    data = get_page( url )
    data = re.sub(r"\n|\r|\t|\s{2}|&nbsp;","",data)
