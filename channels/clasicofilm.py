...
def mainlist(item):
    ...
    itemlist.append(item.clone(action="peliculas", title="      Novedades", url="http://www.clasicofilm.com/feeds/posts/summary?start-index=1&max-results=20&alt=json-in-script&callback=finddatepost",
...
def peliculas(item):
    ...
    data = jsontools.load_json(data)["feed"]
