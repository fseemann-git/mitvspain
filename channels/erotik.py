...
def mainlist(item):
    ...
    itemlist.append( Item(channel=item.channel, action="lista"          , title="Top ultima semana"            , url="http://www.ero-tik.com/topvideos.html?do=recent"))
...
def lista(item):
    ...
    for scrapedurl,scrapedthumbnail, scrapedtitle in matches:
        ...
        # Añade al listado
        itemlist.append(Item(channel=item.channel, action="play", thumbnail=thumbnail ,fanart=thumbnail, title=title , fulltitle=title, url=url,
                             viewmode="movie", folder=True) )

    paginacion = scrapertools.find_single_match(data, '<li class="active"><a href="#" onclick="return false;">\d+</a></li><li class=""><a href="([^"]+)">')

    if paginacion:
        itemlist.append(Item(channel=item.channel, action="lista", title=">> Página Siguiente", url="http://ero-tik.com/"+paginacion))
...
def listacategoria(item):
    ...
    for scrapedurl,scrapedthumbnail, scrapedtitle in matches:
        ...
        # Añade al listado
        itemlist.append(Item(channel=item.channel, action="play", thumbnail=thumbnail , title=title , fulltitle=title, url=url,
                             viewmode="movie", folder=True) )

    paginacion = scrapertools.find_single_match(data, '<li class="active"><a href="#" onclick="return false;">\d+</a></li><li class=""><a href="([^"]+)">')

    if paginacion:
        itemlist.append(Item(channel=item.channel, action="listacategoria", title=">> Página Siguiente", url=paginacion))
