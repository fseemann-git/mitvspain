...
def menuseccion(item):
    ...
    if seccion == '/online/':
    	data = scrapertools.find_single_match(data,'<h2 class="[^"]+"><i class="[^"]+"></i>Películas por géneros<u class="[^"]+"></u></h2>(.*?)<ul class="abc">')
...
def lista (item):
    ...
    for scrapedurl, scrapedtitle, scrapedthumbnail, dataplot in matches:
        ...
        itemlist.append( Item(channel=item.channel, action='findvideos' , title=scrapedtitle , url=scrapedurl, thumbnail=scrapedthumbnail, plot=dataplot, contentTitle = scrapedtitle, extra = item.extra))
    ...
    if itemlist !=[]:
        ...
        matches = re.compile(patron, re.DOTALL).findall(data)
        if matches:
            itemlist.append(item.clone(title="Pagina Siguiente", action='lista', url=urlparse.urljoin(host,matches[0])))
    return itemlist
...
def findvideos(item):
    ...
    listado1 = scrapertools.find_single_match(data,'<div class="links" id="ver-mas-opciones"><h2 class="h2"><i class="[^"]+"></i>[^<]+</h2><ul class="opciones">(.*?)</ul>')
    ...
    for vidId, vidUrl, vidServer, idioma, calidad in matches:
    	itemlist.append( Item(channel=item.channel, action='play', url=vidUrl, extra=vidId, title='Ver en '+vidServer+' | '+idioma+' | '+calidad, thumbnail=item.thumbnail))
    ...
    for vidId, vidUrl, vidServer in matches:
    	itemlist.append( Item(channel=item.channel, action='play', url=vidUrl, extra=vidId, title='Ver en '+vidServer, thumbnail=item.thumbnail))
