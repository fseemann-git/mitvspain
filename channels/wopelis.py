    ...
    for genero in scrapertools.find_multiple_matches(data, '<option value="([^"]+)'):
        if genero != 'Todos':
            ...
            itemlist.append(Item(channel=item.channel, action="listado", title=genero, url=url, text_color=color1,
                      contentType='movie', folder=True, thumbnail=thumbnail))#,viewmode="movie_with_plot"))
    ...
    if itemlist and hasattr(item,'contentSeason'):
        ...
        for i in itemlist:
            if i.infoLabels['title']:
                ...
                i.title = "%sx%s %s" % (i.infoLabels['season'], str(i.infoLabels['episode']).zfill(2), i.infoLabels['title'])
...
def findvideos(item):
    ...
    if itemlist and config.get_library_support() and not "library" in item.extra:
        if item.contentType == 'movie':
            itemlist.append(item.clone(title="Añadir película a la biblioteca",
