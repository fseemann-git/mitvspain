...
def mainlist(item):
    ...
    itemlist = []
    itemlist.append( Item(channel=item.channel, title="Películas"                , action="menupeliculas", url="http://www.bajui.com/descargas/categoria/2/peliculas",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg"))
    itemlist.append( Item(channel=item.channel, title="Series"                   , action="menuseries",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg"))
    itemlist.append( Item(channel=item.channel, title="Documentales"             , action="menudocumentales",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg"))
    itemlist.append( Item(channel=item.channel, title="Buscar"                   , action="search",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg") )
    return itemlist

def menupeliculas(item):
    logger.info("[bajui.py] menupeliculas")
    itemlist = []
    itemlist.append( Item(channel=item.channel, title="Películas - Novedades"        , action="peliculas"   , url=item.url,fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Películas - A-Z"              , action="peliculas"   , url=item.url+"/orden:nombre",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    ...
    for url,title in matches:
        ...
        itemlist.append( Item(channel=item.channel, title="Películas en "+title , action="peliculas", url=scrapedurl,fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))

    itemlist.append( Item(channel=item.channel, title="Buscar"                       , action="search"      , url="",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg") )
    return itemlist

def menuseries(item):
    logger.info("[bajui.py] menuseries")
    itemlist = []
    itemlist.append( Item(channel=item.channel, title="Series - Novedades"           , action="peliculas"        , url="http://www.bajui.com/descargas/categoria/3/series",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Series - A-Z"                 , action="peliculas"        , url="http://www.bajui.com/descargas/categoria/3/series/orden:nombre",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Series - HD"                  , action="peliculas"        , url="http://www.bajui.com/descargas/subcategoria/11/hd/orden:nombre",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Buscar"                       , action="search"            , url="",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg") )
    return itemlist
...
def menudocumentales(item):
    ...
    itemlist = []
    itemlist.append( Item(channel=item.channel, title="Documentales - Novedades"         , action="peliculas"     , url="http://www.bajui.com/descargas/categoria/7/docus-y-tv",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Documentales - A-Z"               , action="peliculas"     , url="http://www.bajui.com/descargas/categoria/7/docus-y-tv/orden:nombre",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot"))
    itemlist.append( Item(channel=item.channel, title="Buscar"                           , action="search"        , url="",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg") )
    return itemlist
...
def peliculas(item,paginacion=True):
    ...
    for title,plot,url,thumbnail in matches:
        ...
        itemlist.append( Item(channel=item.channel, action="enlaces", title=scrapedtitle , fulltitle=title , url=scrapedurl , thumbnail=scrapedthumbnail , plot=scrapedplot , extra=scrapedtitle , context="4|5", fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot") )
    ...
    if len(matches)>0:
        ...
        pagitem = Item(channel=item.channel, action="peliculas", title=">> Página siguiente" , url=scrapedurl,fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg", viewmode="movie_with_plot")
        if not paginacion:
...
def enlaces(item):
    ...
    for thumbnail,usuario,fecha,id,id2,servidores in matches:
        ...
        itemlist.append( Item(channel=item.channel, action="findvideos", title=scrapedtitle , fulltitle=item.title , url=scrapedurl , thumbnail=scrapedthumbnail , plot=scrapedplot , context="4|5",fanart="http://mitvspain.mimediacenter.info/fanart/bajui.jpg") )
