...
def mainlist(item):
    ...
    itemlist.append( item.clone(title="[COLOR floralwhite][B]      Esenciales[/B][/COLOR]", action="scraper",url="https://www.borrachodetorrent.com/peliculas-torrent-deberias-haber-visto/",thumbnail="http://imgur.com/tBvoGIk.png", fanart="http://imgur.com/AqUvMW3.jpg",contentType= "movie"))
    
    itemlist.append( itemlist[-1].clone(title="[COLOR floralwhite][B]Series[/B][/COLOR]", action="scraper",url="https://www.borrachodetorrent.com/series-torrent/",thumbnail="http://imgur.com/lMHcNwc.png",contentType="tvshow"))
    
    itemlist.append( itemlist[-1].clone(title="[COLOR cadetblue][B]Buscar[/B][/COLOR]", action="search",thumbnail="http://imgur.com/NrIVpps.png",fanart="http://imgur.com/AqUvMW3.jpg",))
    return itemlist
...
def findtemporadas(item):
    ...
    if config.get_library_support() and itemlist:
        ...
        itemlist.append(Item(channel= item.channel, title ="Añadir esta serie a la biblioteca", text_color="0xFF00ffff",
...
def epis(item):
    ...
    patron= scrapertools.find_multiple_matches(item.url,'<div class="numerando">([^"]+)</div>.*?href="([^"]+)">([^"]+)</a>')
    ...
    for epi,url,title in patron:
        ...
        itemlist.append(item.clone( title = "[COLOR blue][B]"+epi+"[/B][/COLOR]", url=url,  action="findvideos",show=item.show,fanart=item.extra,extra=item.extra,fanart_extra=item.fanart_extra,fanart_info=item.fanart_info, folder=True ) )
    if item.extra != "serie_add":
        tmdb.set_infoLabels_itemlist(itemlist, __modo_grafico__) 
        for item in itemlist:
...
def findvideos(item):
    ...
    if item.contentType!="movie":
       ...
       if not url and item.library:
          itemlist.append( Item(channel=item.channel, title = "[COLOR slateblue][B]No disponible[/B][/COLOR]", url=url, fanart=fanart,thumbnail= item.thumbnail,extra=item.extra, folder=False) )
       else:
           
        title =  "[COLOR darkturquoise][B]Torrent [/B][/COLOR]"+"[COLOR aliceblue][B]"+capitulo+"[/B][/COLOR]"
        
        title= re.sub(r'\".*','',title)
        itemlist.append( Item(channel=item.channel, title = title, url=url,  action="play",server="torrent", fanart=fanart,thumbnail= item.thumbnail,extra=item.extra, folder=False) )
     ...
     itemlist.append( Item(channel=item.channel, title = "[COLOR deepskyblue][B]Torrent[/B][/COLOR]", url=url,  action="play",server="torrent", fanart=item.fanart,thumbnail= item.thumbnail,extra=item.extra,InfoLabels=item.infoLabels, folder=False) )
     if item.library  and config.get_library_support() and len(itemlist) > 0 :
        ...
        itemlist.append(Item(channel=item.channel, title="Añadir esta película a la biblioteca",
            action="add_pelicula_to_library", url=item.url, infoLabels=infoLabels, text_color="0xFF00ffff",
    ...
    if dd:
        if  item.library:
            extra =  dd
            itemlist.append( Item(channel=item.channel, title = "[COLOR floralwhite][B]      Online[/B][/COLOR]"  , url=item.url,  action="dd_y_o", thumbnail="http://imgur.com/hYgra9W.png", fanart=item.extra.split("|")[0],contentType=item.contentType, extra=str(extra)+"|"+item.extra, folder=True) )
        else:
                        
            patron=scrapertools.find_multiple_matches (str(dd),'<li class="elemento">.*?href="([^"]+)".*?<span class="c">([^"]+)</span>.*?<span class="d">([^"]+)</span>')
            ...
            for url,idioma,calidad, in patron:
                ...
                for video in videolist:
                   ...
                   if not os.path.exists(icon_server):
                    icon_server = ""
                   itemlist.append(Item(channel=item.channel ,url=video.url, server=video.server,title="[COLOR floralwhite][B]"+video.server+"[/B][/COLOR]"+" "+"[COLOR powderblue]"+idioma+"[/COLOR]"+"[COLOR deepskyblue]--"+calidad+"[/COLOR]",thumbnail=icon_server, fanart=fanart,action="play", folder=False) )
    if not item.infoLabels['episode'] and item.contentType!="movie":
          itemlist.append( Item(channel=item.channel,title = "[COLOR paleturquoise][B]Todos los episodios[/B][/COLOR]", url=url_serie,  action="findtemporadas",server="torrent", fanart=item.extra.split("|")[1],thumbnail=item.thumbnail,thumb_art= item.thumb_art,thumb_info=item.thumb_info,extra=item.extra+"|"+item.thumbnail,contentType=item.contentType,contentTitle=item.contentTitle, InfoLabels=item.infoLabels,library=item.library,fulltitle=item.fulltitle, folder=True) ) 
...
def dd_y_o(item):
    ...
    if item.contentType=="movie":
       ...
       enlaces=item.extra.split("|")[0]
       fanart= ""
    patron=scrapertools.find_multiple_matches (enlaces,'<li class="elemento">.*?href="([^"]+)".*?<span class="c">([^"]+)</span>.*?<span class="d">([^"]+)</span>')
    for url,idioma,calidad, in patron:
        idioma = re.sub(r'\\xc3\\xb1','ñ',idioma)
        idioma = re.sub(r'\\xc3\\xa9','é',idioma)
        videolist = servertools.find_video_items(data=url)
     ...
     if item.contentType=="movie":
         ...
         if len(imagenes)>= 6:
            ...
            if imagenes[0]!=check_fanart:
               ...
               item.fanart= imagenes[1]
            if imagenes[1] != check_fanart and imagenes[1] != item.fanart and imagenes[2] != check_fanart:
               item.extra= imagenes[1]+"|"+imagenes[2]+"|"+imagenes[3]+"|"+imagenes[4]+"|"+imagenes[5]
                   
            else: 
               if imagenes[1] != check_fanart and imagenes[1] != item.fanart:
                  item.extra= imagenes[1]+"|"+imagenes[3]+"|"+imagenes[4]+"|"+imagenes[5]+"|"+imagenes[2]
               elif imagenes[2] != check_fanart:
                    item.extra= imagenes[2]+"|"+imagenes[3]+"|"+imagenes[4]+"|"+imagenes[5]+"|"+imagenes[1]
               else:
                   item.extra= imagenes[3]+"|"+imagenes[4]+"|"+imagenes[5]+"|"+imagenes[2]+"|"+imagenes[1]
         elif len(imagenes) ==5:
              if imagenes[0]!=check_fanart:
               item.fanart= imagenes[0]
              else:
