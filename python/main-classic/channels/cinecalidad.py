...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Audio Latino", action="submenu", host="http://cinecalidad.com/",
                         thumbnail=thumbmx, extra="peliculas"))
    itemlist.append(
        Item(channel=item.channel, title="Audio Castellano", action="submenu", host="http://cinecalidad.com/espana/",
             thumbnail=thumbes, extra="peliculas"))
    itemlist.append(
        Item(channel=item.channel, title="Audio Portugues", action="submenu", host="http://cinemaqualidade.com/",
             thumbnail=thumbbr, extra="filmes"))
        ...
        if dec(scrapedurl) in servidor:
            ...
            if 'yourupload' in dec(scrapedurl):
                ...
                for video in url_list:
                    ...
                    itemlist.append(Item(channel=item.channel, action="play", title=new_title, fulltitle=item.title,
                                         url=video['file'], thumbnail=item.thumbnail, plot = item.plot,
                                         server='directo'))
            ...
            if title not in duplicados and url != '':
                itemlist.append(Item(channel=item.channel, action="play", title=title, fulltitle=item.title, url=url,
                                     thumbnail=thumbnail, plot=item.plot, extra=item.thumbnail,
