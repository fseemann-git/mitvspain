...
def mainlist(item):
    ...
    itemlist.append( Item(channel=item.channel, title="[COLOR forestgreen]Series[/COLOR]" , action="scraper", url="http://cuelgame.net/?category=8" ,thumbnail="http://imgur.com/OjP42lL.jpg", fanart="http://imgur.com/Xm49VbL.jpg"))
    itemlist.append( Item(channel=item.channel, title="[COLOR forestgreen]TV[/COLOR]" , action="scraper", url="http://cuelgame.net/?category=67" ,thumbnail="http://imgur.com/C4VDnTo.png", fanart="http://imgur.com/LDoJrAf.jpg"))
    itemlist.append( Item(channel=item.channel, title="[COLOR forestgreen]Documentales[/COLOR]" , action="scraper", url="http://cuelgame.net/?category=68" ,thumbnail="http://imgur.com/nofNYjy.jpg", fanart="http://imgur.com/upB1jL8.jpg"))
    itemlist.append( Item(channel=item.channel, title="[COLOR forestgreen]Música[/COLOR]" , action="scraper", url="http://cuelgame.net/?category=13" ,thumbnail="http://imgur.com/DPrOlme.jpg", fanart="http://imgur.com/FxM6xGY.jpg"))

    itemlist.append( Item(channel=item.channel, title="[COLOR forestgreen]Buscar[/COLOR]"   , action="search", url="", thumbnail="http://images2.alphacoders.com/846/84682.jpg", fanart="http://imgur.com/1sIHN1r.jpg"))
...
def scraper(item):
   ...
   <h2> <a href="magnet:?xt=urn:btih:4E7192D0885DDB9219699BBFFD72E709006BF9F2&amp;dn=automata+2014+hdrip+xvid+sam+etrg&amp;tr=udp%3A%2F%2Fopen.demonii.com%3A1337%2Fannounce" class="l:12087"onmousedown="return clk(this, 12087)" >Automata (2014) [HDRip] [VO] </a><img src="http://cuelgame.net/img/common/is-magnet.png" class="media-icon" width="18" height="15" alt="magnet" title="magnet" /> <a href="magnet:?xt=urn:btih:4E7192D0885DDB9219699BBFFD72E709006BF9F2&dn=automata+2014+hdrip+xvid+sam+etrg&tr=udp%3A%2F%2Fopen.demonii.com%3A1337%2Fannounce" title="Direct link" rel="nofollow, noindex"><img src="http://cuelgame.net/img/common/link-02.png" class="media-icon" width="18" height="15" alt="Enlace directo" title="Enlace directo" /></a> </h2> <div class="news-submitted"><a href="/user/Dios" class="tooltip u:1179"><img src="http://cuelgame.net/cache/00/04/1179-1328100564-25.jpg" width="25" height="25" alt=""/></a><strong>magnet:?xt=urn:btih:4E7192D0885DDB9219699BBFFD72E709006BF...</strong><br /> por<a href="/user/Dios/history">Dios</a> hace7 horaspublicado hace5 horas 58 minutos</div><img src='http://cuelgame.net/cache/00/2f/thumb-12087.jpg' width='70' height='70' alt='' class='thumbnail'/><p> En un futuro no lejano, en el que el planeta Tierra sufre una creciente desertización, Jacq Vaucan (Antonio Banderas), un agente de seguros de una compañía de robótica, investiga un caso en apariencia rutinario cuando descubre algo que podría tener consecuencias decisivas para el futuro de la humanidad. Banderas produce y protagoniza este thriller futurista, que especula sobre lo que ocurriría si la inteligencia artificial superase a la humana.|<i> Más info. en comentarios.</i></p>
    ...
    for scrapedurl, scrapedtitle,check_thumb, scrapedplot,check_multimedia in matches:
        ...
        title_fan= re.sub(r"End Part \d|\||\[.*?\].*|\(.*?\).*|\d+x\d+.*?Final|-\d+|\d+x\d+|Temporada.*?Completa| ;|V.O|\d.*?GB|\+|subs|s\d+e\d+p.*|s\d+e\d+i.*|s\d+e\d+.*|S\d+E\d+[^<]+|VO|Serie.*|S\d+E\d+p.*|S\d+E\d+720p.*","",scrapedtitle)
        ...
        if "category=4" in item.url or "category=8" in item.url:
           ...
           itemlist.append( Item(channel=item.channel, title=scrapedtitle, url="", action="fanart", server="torrent", thumbnail=scrapedthumbnail, extra=extra, folder=True) )
        else:
          
          itemlist.append( Item(channel=item.channel, title=scrapedtitle, url=scrapedurl, action="play", server="torrent", thumbnail=scrapedthumbnail, folder=False) )
    ...
    if len(matches)>0:
       ...
       itemlist.append( Item(channel=item.channel, action="scraper", title="Página siguiente >>" , url=scrapedurl , thumbnail = "http://imgur.com/ycPgVVO.png",folder=True) )
    ...
    if "category=4" in check_sp:
        ...
        if len(matches)==0:
                ...
                if len(matches)==0:
                    ...
                    itemlist.append( Item(channel=item.channel, title=item.title, url=item.url, action="play", server="torrent",thumbnail=item.thumbnail, fanart=item.fanart,extra = extra, show= show, category= category,folder=False) )
        ...
        for id, fan in matches:
            ...
            if len(matches)==0:
                ...
                itemlist.append( Item(channel=item.channel, title = item.title , action="play", url=item.url, server="torrent", thumbnail=posterdb, fanart=item.extra, extra=extra, show=show, category= category, folder=False) )
            for logo in matches:
                if '"hdmovieclearart"' in data:
                    ...
                    if '"moviebackground"' in data:
                        ...
                        if '"moviedisc"' in data:
                            ...
                            category= clear
                        itemlist.append( Item(channel=item.channel, title = item.title , action="play", url=item.url, server="torrent", thumbnail=logo, fanart=item.extra, extra=extra,show=show,  category= category,folder=False) )
                    else:
                        extra= clear
                        show=fanart_2+"|"+fanart_3+"|"+sinopsis
                        if '"moviedisc"' in data:
        ...
        if len(matches)==0:
         ...
         if len(matches)==0:
          ...
          if len(matches)==0:
            ...
            posterdb= item.thumbnail
            if temp_epi:
               itemlist.append( Item(channel=item.channel, title=item.title, url=item.url, action="play", thumbnail=item.thumbnail, fanart=item.fanart ,server = "torrent", extra=extra, category= category,  show=show , folder=False) )
            else:
                itemlist.append( Item(channel=item.channel, title=item.title, url=item.url, action="findvideos", thumbnail=item.thumbnail, fanart=item.fanart ,fulltitle=fulltitle, extra=extra, category= category,  show=show , folder=True) )
                ...
                if '"hdtvlogo"' in data:
                    if "showbackground" in data:
                        ...
                        if '"hdclearart"' in data:
                            ...
                            show = fanart_2+"|"+fanart_3+"|"+sinopsis+"|"+title_fan+"|"+tfv+"|"+id_tmdb
                        if temp_epi:
                           itemlist.append( Item(channel=item.channel, title = item.title , action="play", url=item.url,server="torrent", thumbnail=thumbnail, fanart=item.extra, category=category, extra=extra, show=show, folder=False) )
                        else:
                           itemlist.append( Item(channel=item.channel, title = item.title , action="findvideos", url=item.url,fulltitle=temp+"|"+epi, thumbnail=thumbnail, fanart=item.extra, category=category, extra=extra, show=show, folder=True) )
                        ...
                        if temp_epi:
                            ...
                            itemlist.append( Item(channel=item.channel, title = item.title , action="findvideos", url=item.url,fulltitle=temp+"|"+epi, thumbnail=thumbnail, fanart=item.extra, extra=extra, show=show,  category= category, folder=True) )
                else:
                    extra=  ""+"|"+year
                    show = fanart_2+"|"+fanart_3+"|"+sinopsis+"|"+title_fan+"|"+tfv+"|"+id_tmdb
                    if temp_epi:
                       itemlist.append( Item(channel=item.channel, title = item.title , action="play", url=item.url,  server="torrent", thumbnail=posterdb, fanart=fanart, extra=extra, show=show, category = category, folder=False) )
                    else:
                       itemlist.append( Item(channel=item.channel, title = item.title , action="findvideos", url=item.url,  fulltitle=temp+"|"+epi, thumbnail=posterdb, fanart=fanart, extra=extra, show=show, category = category, folder=True) )
            ...
            for logo in matches:
                ...
                if '"clearart"' in data:
                    ...
                    if "showbackground" in data:
                        ...
                        show= fanart_2+"|"+fanart_3+"|"+sinopsis+"|"+title_fan+"|"+tfv+"|"+id_tmdb
                        if temp_epi:
                           itemlist.append( Item(channel=item.channel, title = item.title , action="play", url=item.url, server="torrent", thumbnail=thumbnail, fanart=item.extra, extra=extra,show=show, category= category,  folder=False) )
                        else:
                            itemlist.append( Item(channel=item.channel, title = item.title , action="findvideos", url=item.url, fulltitle=temp+"|"+epi, thumbnail=thumbnail, fanart=item.extra, extra=extra,show=show, category= category,  folder=True) )
...
def info(item):
    ...
    item_info = item.clone(info=infoLabels, icon=icon, extra=id, rating=rating, rating_filma=rating_filma, critica=critica, contentType=check2, thumb_busqueda="http://imgur.com/HqI3JnO.png")
