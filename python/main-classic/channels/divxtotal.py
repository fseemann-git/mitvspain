...
def scraper(item):
    ...
    if len(next)>0:
        ...
        itemlist.append(item.clone( title="[COLOR springgreen][B]Siguiente >>[/B][/COLOR]", thumbnail="http://imgur.com/a7lQAld.png",url=url))
    try:
        from core import tmdb
        tmdb.set_infoLabels_itemlist(itemlist, __modo_grafico__)
        for item in itemlist:
...
def findtemporadas(item):
    ...
    if config.get_library_support() and itemlist:
        ...
        itemlist.append(Item(channel= item.channel, title ="Añadir esta serie a la biblioteca", text_color="0xFFe5ffcc",
...
def epis(item):
    ...
    for idioma,url,epi in patron:
        ...
        itemlist.append(item.clone( title = "[COLOR orange]"+epi+"[/COLOR]"+"[COLOR sandybrown] "+idioma+"[/COLOR]", url=url,  action="findvideos",show=item.show,fanart=item.extra,extra=item.extra,fanart_extra=item.fanart_extra,fanart_info=item.fanart_info, folder=True ) )
    if item.extra != "serie_add":
       tmdb.set_infoLabels_itemlist(itemlist, __modo_grafico__) 
       for item in itemlist:
...
def findvideos(item):
    ...
    if item.contentType!="movie":
       ...
       if not item.infoLabels['episode'] :
          ...
          url_capitulo = scrapertools.find_single_match(data,'<a href="(http://www.divxtotal.com/wp-content/uploads/.*?'+capitulo+'.*?.torrent)')
       ...
       itemlist.append( Item(channel=item.channel, title = "[COLOR chocolate][B]Ver capítulo "+capitulo+"[/B][/COLOR]"+"-"+"[COLOR khaki] ( Video"+"[/COLOR]"+" "+"[COLOR khaki]"+ext_v+"[/COLOR]"+" "+"[COLOR khaki] "+size+" )"+"[/COLOR]", url=url_capitulo,  action="play",server="torrent", fanart=fanart,thumbnail= item.thumbnail,extra=item.extra,fulltitle=item.fulltitle, folder=False) )
       ...
       if item.infoLabels['episode'] and item.library :
           ...
           if not "assets.fanart" in item.fanart_info:
              ...
              fanart = item.fanart
           itemlist.append( Item(channel=item.channel, title = "[COLOR darksalmon][B]       info[/B][/COLOR]",  action="info_capitulos", fanart=fanart,thumbnail= item.thumb_art,thumb_info=item.thumb_info,extra=item.extra, show= item.show,InfoLabels= item.infoLabels, folder=False) )
