...
def mainlist(item):
    ...
    itemlist.append( itemlist[-1].clone(title="   [COLOR lightskyblue][B]Recomendadas[/B][/COLOR]", action="scraper",url="http://peliculasgratis.biz/catalogue?order=most_rated",thumbnail="http://imgur.com/fN2p6qH.png.png", fanart="http://imgur.com/b8OuBR2.jpg",contentType= "movie"))
    itemlist.append( itemlist[-1].clone(title="[COLOR lightskyblue][B]   Actualizadas[/B][/COLOR]", action="scraper",url="http://peliculasgratis.biz/catalogue?",thumbnail="http://imgur.com/fN2p6qH.png", fanart="http://imgur.com/b8OuBR2.jpg",contentType= "movie"))
    itemlist.append( itemlist[-1].clone(title="[COLOR lightskyblue][B]Series[/B][/COLOR]", action="scraper",url="http://peliculasgratis.biz/lista-de-series",thumbnail="http://imgur.com/Jia27Uc.png",fanart="http://imgur.com/b8OuBR2.jpg",contentType="tvshow"))
    itemlist.append( itemlist[-1].clone(title="[COLOR lightskyblue][B]Buscar[/B][/COLOR]", action="",url="",thumbnail="http://imgur.com/mwTwfN7.png", fanart="http://imgur.com/b8OuBR2.jpg"))
    itemlist.append( itemlist[-1].clone(title="[COLOR lightskyblue][B]   Buscar Película[/B][/COLOR]", action="search",url="",thumbnail="http://imgur.com/mwTwfN7.png", fanart="http://imgur.com/b8OuBR2.jpg",contentType="movie"))
    itemlist.append( itemlist[-1].clone(title="[COLOR lightskyblue][B]   Buscar Serie[/B][/COLOR]", action="search",url="",thumbnail="http://imgur.com/mwTwfN7.png", fanart="http://imgur.com/b8OuBR2.jpg",contentType="tvshow"))
...
def scraper(item):
    ...
    if item.contentType!="movie":
       matches = scrapertools.find_multiple_matches(bloque_enlaces,'<a class="i" href="([^"]+)".*?src="([^"]+)".*?<div class="l">(.*?)<\/a><h3>.*?(completa)">([^"]+)<\/a><\/h3> <span>(.*?)<\/span>')
    else:
       matches = scrapertools.find_multiple_matches(bloque_enlaces,'<a class="i" href="([^"]+)".*?src="([^"]+)".*?">([^<]+)<.*?<div class="l">(.*?)<\/a><h3>.*?title[^<]+>([^<]+)<\/a><\/h3> <span>(.*?)<')
    ...
    if check_year:
     ...
     if len(next)>0:
        ...
        itemlist.append( Item(channel=item.channel, action="scraper", title="[COLOR floralwhite][B]Siguiente[/B][/COLOR]" , url=url , thumbnail="http://imgur.com/jhRFAmk.png", fanart="http://imgur.com/nqmJozd.jpg",extra=item.extra,contentType=item.contentType,  folder=True) )
...
def findvideos(item):
    ...
    if item.extra !="dd" and item.extra != "descarga" :
     ...
     if item.contentType!="movie":
        ...
        if bloque_links=="":
          bloque_links=scrapertools.find_single_match(data,'<div class="links">(.*?)<div class="enlaces">')
     links=scrapertools.find_multiple_matches(bloque_links,'<a class="goto" rel="nofollow".*?data-id="([^<]+)".*?src="([^"]+)">([^<]+)<.*?src="([^"]+)">([^<]+).*?<span>([^<]+)')
     for id, thumb,server,idiomapng,idioma,calidad in links:
        ...
        icon_server = os.path.join( config.get_runtime_path() , "resources" , "images" , "servers" , "server_"+server_name+".png" )
        ...
        extra="online"
        itemlist.append( Item(channel=item.channel, title = "[COLOR lightcyan][B]"+server+"[/B][/COLOR] "+calidad+" "+idioma  , url=url,  action="play", thumbnail=icon_server, fanart="",contentType=item.contentType, folder=True, id = url_post) )
    else:
     bloque_dd=scrapertools.find_single_match(data,'<\/i>Descargar(.*?)<div class="enlaces">')
     links_dd=scrapertools.find_multiple_matches(bloque_dd,'<a class="low".*?data-id="(.*?)".*?src="([^"]+)">([^<]+)<.*?src[^<]+>([^<]+).*?<span>([^<]+)')
     for id, thumb,server,idioma,calidad in links_dd:
        ...
        if not os.path.exists(icon_server):
            icon_server = thumb
        extra="descarga"
        itemlist.append(item.clone( title = "[COLOR floralwhite][B]"+server+"[/B][/COLOR] "+calidad+" "+idioma  , url=url,  action="play", thumbnail=icon_server, fanart="",contentType=item.contentType,id=data_post) )
    ...
    if item.contentType =="movie" and item.extra != "descarga" and item.extra != "online":
       ...
       if config.get_library_support() and len(itemlist) > 0:
         itemlist.append(Item(channel=item.channel, title="Añadir película a la biblioteca",
                                     ...
                                     infoLabels={'title': item.fulltitle},thumbnail="http://imgur.com/xjrGmVM.png", fulltitle=item.fulltitle,
                                     extra=extra))
    if item.extra !="dd" and item.extra != "descarga" and item.extra != "online" :
      bloque_dd=scrapertools.find_single_match(data,'<\/i>Descargar(.*?)<div class="enlaces">')
      if bloque_dd:
         itemlist.append(item.clone(  title = "[COLOR aqua][B]Ver enlaces Descarga[/B][/COLOR] ",  action="findvideos", thumbnail=thumb, fanart="",contentType=item.contentType, bloque_dd=bloque_dd, extra="dd") )
