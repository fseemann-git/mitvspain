...
def entradasconlistas(item):
    ...
    if itemlist:
        itemlist.insert(0, Item(channel=item.channel, title="**LISTAS**", action="", text_color=color4, text_blod=True,
                                thumbnail=item.thumbnail, fanart=item.fanart))

    if data.get("a") and itemlist:
        itemlist.append(Item(channel=item.channel, title="**VÍDEOS**", action="", text_color=color6, text_blod=True,
                             thumbnail=item.thumbnail, fanart=item.fanart))
    ...
    if data.get('a') and itemlist and contentList and config.get_library_support():
        itemlist.append(Item(channel=item.channel, text_color=color5, title="Añadir esta lista a la biblioteca",
                             url=item.url, action="listas"))
    elif contentSerie and config.get_library_support():
        itemlist.append(Item(channel=item.channel, title="Añadir esta serie a la biblioteca", text_color=color5,
...
def findvideos(item):
    ...
    if item.video_urls:
        ...
        if itemlist and item.extra == "" and config.get_library_support():
            itemlist.append(Item(channel=item.channel, title="Añadir enlaces a la biblioteca", text_color=color5,
...
def listas(item):
    ...
    if not error:
        itemlist.append(Item(channel=item.channel, title='Lista añadida correctamente a la biblioteca', action=""))
    else:
