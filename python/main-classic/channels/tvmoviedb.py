...
def trakt(item):
    ...
    if not item.extra:
        ...
        itemlist.append(item.clone(title="     Viéndose Ahora", action="acciones_trakt", url="movies/trending%s" % page))
        itemlist.append(item.clone(title="     Más Vistas", action="acciones_trakt", url="movies/watched/all%s" % page))
        itemlist.append(item.clone(title="     Más  Esperadas", action="acciones_trakt", url="movies/anticipated%s" % page))
        if token_auth:
        ...
        itemlist.append(item.clone(title="     Más  Esperadas", action="acciones_trakt", url="shows/anticipated%s" % page))
        if token_auth:
            itemlist.append(item.clone(title="     Recomendaciones personalizadas", action="acciones_trakt",
        ...
        if not token_auth:
            ...
            itemlist.append(item.clone(title="     Películas", action="acciones_trakt", url="users/me/watchlist/movies%s" % page,
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="     Series", action="acciones_trakt", url="users/me/watchlist/shows%s" % page, extra="show",
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="Vistas", action="", text_color=color2))
            itemlist.append(item.clone(title="     Películas", action="acciones_trakt", url="users/me/watched/movies%s" % page,
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="     Series", action="acciones_trakt", url="users/me/watched/shows%s" % page, extra="show",
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="En mi Colección", action="", text_color=color2))
            itemlist.append(item.clone(title="     Películas", action="acciones_trakt", url="users/me/collection/movies%s" % page,
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="     Series", action="acciones_trakt", url="users/me/collection/shows%s" % page, extra="show",
                                       order="added", how="desc"))
            itemlist.append(item.clone(title="Mis listas", action="acciones_trakt", url="users/me/lists", text_color=color2))
...
def mal(item):
    ...
    itemlist.append(item.clone(title="Top Series", url="https://myanimelist.net/topanime.php?type=tv&limit=0", action="top_mal",
                               contentType="tvshow", extra="tv"))
    itemlist.append(item.clone(title="Top Películas", url="https://myanimelist.net/topanime.php?type=movie&limit=0", action="top_mal",
                               contentType="movie", extra="movie"))
    itemlist.append(item.clone(title="Top Ovas", url="https://myanimelist.net/topanime.php?type=ova&limit=0", action="top_mal",
                               contentType="tvshow", extra="tv", tipo="ova"))
    itemlist.append(item.clone(title="Más Populares", url="https://myanimelist.net/topanime.php?type=bypopularity&limit=0", action="top_mal"))
    itemlist.append(item.clone(title="Más Esperados", url="https://myanimelist.net/topanime.php?type=upcoming&limit=0", action="top_mal"))
...
def listado_tmdb(item):
    ...
    if "collection" in item.search["url"]:
            ...
            for i in range(0, len(ob_tmdb.results)):
                ...
                if not 'person' in item.search["url"] or 'tv_credits' in item.search["url"]:
                    ...
                    if known_for:
                        ...
                        new_item.title = "%s  [COLOR %s](%s)[/COLOR]" \
                                         % (new_item.contentTitle, color6, known_for[random].get("title", known_for[random].get("name")))
        ...
        if "page" in item.search and ob_tmdb.total_pages > item.search["page"]:
            item.search["page"] += 1
            itemlist.append(Item(channel=item.channel, action=item.action, title=">> Página siguiente",
                                 search=item.search, extra=item.extra, pagina=item.pagina+1, contentType=item.contentType))
...
def detalles(item):
    ...
    if not item.infoLabels["tmdb_id"]:
        ...
        data = httptools.downloadpage("http://www.imdb.com/title/" + item.infoLabels['imdb_id'], headers=headers, replace_headers=True).data
    ...
    try:
        if ob_tmdb.result.get("belongs_to_collection"):
            ...
            itemlist.append(new_item.clone(title="Es parte de: %s" % saga["name"], action="listado_tmdb",
                                           text_color=color5))
    except:
        pass
    ...
    if item.infoLabels['tmdb_id']:
        ...
        itemlist.append(item.clone(title="%s similares" % title, action="listado_tmdb",
                                   ...
                                   'language': langt, 'page': 1}, infoLabels={'mediatype': item.contentType}, text_color=color2))
        itemlist.append(item.clone(title="Recomendaciones", action="listado_tmdb", infoLabels={'mediatype': item.contentType},
...
def info_seasons(item):
    ...
    for temp in range(item.infoLabels["number_of_seasons"], 0, -1):
        ...
        if temporada:
            ...
            new_item.title = "Temporada %s" % temp
            itemlist.append(new_item)
...
def indices_tmdb(item):
    ...
    if "Géneros" in item.title:
        ...
        url = ('http://api.themoviedb.org/3/genre/%s/list?api_key=f7f51775877e0bb6703520952b3c7840&language=%s'
               % (item.extra, langt))
        try:
            lista = jsontools.load_json(httptools.downloadpage(url, cookies=False).data)["genres"]
            lista_generos = {}
            for l in lista:
        ...
        for key, value in lista_generos.items():
            ...
            new_item.search = {'url': 'discover/%s' % item.extra, 'with_genres': key, 'sort_by': sort_by, param_year: fecha,
                               'language': langt, 'page': 1}
            itemlist.append(new_item)
...
def listado_fa(item):
    ...
    if next_page:
        ...
        itemlist.append(item.clone(title=">> Página Siguiente", text_color=""))
    return itemlist
...
def detalles_fa(item):
    ...
    if mivoto:
        ...
        new_item = item.clone(action="votar_fa", title="[FA] Mi voto: %s  ---> ¿Cambiar?" % mivoto,
                              itk=itk, voto=int(mivoto), folder=folder)
        new_item.infoLabels["duration"] = ""
        itemlist.append(new_item)
    else:
        if itk:
            new_item = item.clone(action="votar_fa", title="[FA] Votar %s" % title, itk=itk, accion="votar",
                                  folder=folder)
            new_item.infoLabels["duration"] = ""
            itemlist.append(new_item)
    ...
    if itk:
        ...
        new_item = item.clone(action="acciones_fa", accion="lista_movie", itk=itk,
                              title="[FA] Añadir o quitar de una lista de usuario")
        new_item.infoLabels["duration"] = ""
        itemlist.append(new_item)
    ...
    if ob_tmdb.result:
        ...
        itemlist.append(item.clone(title="%ss similares" % title.capitalize(), action="listado_tmdb",
                                   search={'url': '%s/%s/similar' % (item.extra, item.infoLabels['tmdb_id']),
                                   'language': langt, 'page': 1}, infoLabels={'mediatype': item.contentType}, text_color=color2))
        itemlist.append(item.clone(title="Recomendaciones", action="listado_tmdb", infoLabels={'mediatype': item.contentType},
                                   search={'url': '%s/%s/recommendations' % (item.extra, item.infoLabels['tmdb_id']),
...
def login_fa():
    ...
    try:
        ...
        if user == "" or password == "":
            return False, "Usuario y/o contraseñas no configurados"
        ...
        if "Invalid username" in data:
            ...
            data = httptools.downloadpage("http://m.filmaffinity.com/%s/tpl.ajax.php?action=getTemplate" % langf, post).data
...
def cuenta_fa(item):
    ...
    if not login:
        ...
        userid = config.get_setting("userid", "tvmoviedb")
        itemlist.append(item.clone(action="acciones_fa", title="Mis votaciones", text_color=color5, accion="votos",
                                   url="http://m.filmaffinity.com/%s/user_ratings.php?id-user=%s" % (langf, userid)))
        itemlist.append(item.clone(action="acciones_fa", title="Mis listas", text_color=color5,  accion="listas",
...
def acciones_fa(item):
    ...
    if item.accion == "votos" or item.accion == "lista":
        ...
        data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
        data = re.sub(r"\s{2}", " ", data)
        bloques = scrapertools.find_multiple_matches(data, 'list-group-item(?:[^>]+>\s*<a| rip)(.*?</div>)\s*</div>\s*<div')
        for bloque in bloques:
            ...
            for url, thumb, title, year, rating, votos in matches:
                ...
                if re.search(r'(?i)serie de tv|\(tv\)', title):
                    new_item.contentType = "tvshow"
                    new_item.extra = "tv"
                new_item.title = title.strip() + "  (%s) [COLOR %s]%s[/COLOR]/[COLOR %s]%s[/COLOR]" % (year, color6, rating, color4, mivoto)
        ...
        for listid, chequeo, title in matches:
            ...
            itemlist.append(new_item)
        new_item = item.clone(action="newlist", title="Añadir una nueva lista", text_color=color6)
        new_item.infoLabels["duration"] = ""
        itemlist.append(new_item)
    else:
...
def newlist(item):
    ...
    if item.accion == "lista":
        ...
        itemlist.append(item.clone(action="", title="Lista creada correctamente"))
    else:
...
def imagenes(item):
    ...
    if item.extra == "menu":
        ...
        if "myanimelist" in item.images:
            ...
            images = scrapertools.find_multiple_matches(data, '<div class="picSurround"><a href="([^"]+)" title="([^"]+)"')
            if images:
    ...
    if "Fanart" in item.title:
        ...
        if not resultado:
            itemlist.append(item.clone(title="Web de Fanart.tv no disponible. Vuelve a intentarlo", action=""))
            return itemlist
        ...
        try:
            ...
            itemlist.append(item.clone(title="No hay imágenes disponibles", action=""))
            return itemlist
    elif "Imdb" in item.title:
        try:
            data = jsontools.load_json(httptools.downloadpage(item.images["imdb"]["url"], cookies=False).data)
...
def fanartv(item):
    ...
    if id_search:
        ...
        data = jsontools.load_json(httptools.downloadpage(url, headers=headers, replace_headers=True).data)
...
def menu_trakt(item):
    ...
    try:
        ...
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(item.clone(title="[Trakt] Marcar %s como no vista" % title, action="acciones_trakt",
                                       url="sync/history/remove", post=post))
        else:
            itemlist.append(item.clone(title="[Trakt] Marcar %s como vista" % title, action="acciones_trakt",
    ...
    try:
        ...
        post = {tipo: [{"ids": {"tmdb": item.infoLabels["tmdb_id"]}}]}
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(item.clone(title="[Trakt] Eliminar %s de tu watchlist" % title, action="acciones_trakt",
                                       url="sync/watchlist/remove", post=post))
        else:
            itemlist.append(item.clone(title="[Trakt] Añadir %s a tu watchlist" % title, action="acciones_trakt",
    ...
    try:
        ...
        post = {tipo: [{"ids": {"tmdb": item.infoLabels["tmdb_id"]}}]}
        if '"tmdb":%s' % item.infoLabels["tmdb_id"] in result:
            itemlist.append(item.clone(title="[Trakt] Eliminar %s de tu colección" % title, action="acciones_trakt",
                                       url="sync/collection/remove", post=post))
        else:
            itemlist.append(item.clone(title="[Trakt] Añadir %s a tu colección" % title, action="acciones_trakt",
...
def acciones_trakt(item):
    ...
    post = None
    if item.post:
        post = jsontools.dump_json(item.post)
    ...
    if data and "sync" in item.url:
        if not item.post:
            ...
            if "not_found" in data:
                ...
                return platformtools.dialog_notification("Trakt", "Hubo error en el proceso")
        ...
        try:
            for i, entry in enumerate(data):
                ...
                itemlist.append(new_item)
                if i == item.pagina + 20:
                    itemlist.append(item.clone(title=">> Página Siguiente", text_color="", pagina=item.pagina+20))
                    break
        ...
        if data and "page=1" in item.url and item.order:
            ...
            itemlist.append(item.clone(title="Lista ordenada por %s. ¿Cambiar orden?" % orden, action="order_list",
                                       text_color=color4))
        ratings = []
        try:
            if item.order:
        ...
        if "page" in item.url and len(itemlist) == 20:
            ...
            itemlist.append(item.clone(title=">> Página Siguiente", text_color="", url=url))
    else:
        data = jsontools.load_json(data)
        for entry in data:
            new_item = item.clone()
...
def detalles_mal(item):
    ...
    if not item.infoLabels['duration']:
        try:
            horas, min1, min2 = scrapertools.find_single_match(data, 'Duration:</span>\s*(?:(\d+) hr\. (\d+) min|(\d+) min)')
            if horas:
                horas = int(horas) * 360
            else:
                horas = 0
    ...
    if item.infoLabels["title"] and title_mal != item.infoLabels["title"]:
        itemlist.append(item.clone(action="busqueda", contentTitle=item.infoLabels["title"],
                                   title="    Buscar por título alternativo: %s" % item.infoLabels["title"]))
    ...
    if eng_title and item.contentTitle != eng_title and title_mal != eng_title:
        itemlist.append(item.clone(action="busqueda", contentTitle=eng_title,
                                   title="    Buscar por su título en inglés: %s" % eng_title))

    if item_tmdb.extra == "tv" and ob_tmdb.result:
        itemlist.append(item.clone(action="info_seasons", text_color=color4,
                                   title="Info de temporadas [%s]" % item.infoLabels["number_of_seasons"]))

    itemlist.append(item.clone(action="videos_mal", title="Vídeos (Episodios, Trailers...)", text_color=color5,
                               url=item.url+"/video"))
    ...
    if not "No characters or voice actors" in data and not "No staff for this anime" in data:
        itemlist.append(item.clone(action="staff_mal", title="Personajes/Staff", text_color=color2,
    ...
    if item.login and "Add to My List</span>" in data and config.is_xbmc():
        itemlist.append(item.clone(title="[MAL] Añadir a tus listas/%s" % score, action="menu_mal", contentTitle=title_mal))
    elif item.login and config.is_xbmc():
        status = {'1': 'Viendo Actualmente', '2': 'Completados', '3': 'En pausa', '4': 'Descartados', '6': 'Previstos para ver'}
        estado = scrapertools.find_single_match(data, 'myinfo_updateInfo".*?option selected="selected" value="(\d+)"')
        try:
            estado = status[estado]
            itemlist.append(item.clone(title="[MAL] En tu lista de [COLOR %s]%s[/COLOR]. ¿Cambiar?/%s" % (color6, estado, score), action="menu_mal",
    ...
    if ob_tmdb.result:
        ...
        itemlist.append(item.clone(title="Recomendaciones TMDB", action="listado_tmdb", infoLabels={'mediatype': item.contentType},
                                   search={'url': '%s/%s/recommendations' % (item.extra, item.infoLabels['tmdb_id']),
                                   'language': langt, 'page': 1}, text_color=color2))

    # Recomendaciones myanimelist y búsqueda de info en anidb (fansubs en español)
    itemlist.append(item.clone(title="Recomendaciones MyAnimeList", action="reco_mal"))
    anidb_link = scrapertools.find_single_match(data, '<a href="(http://anidb.info/perl-bin/animedb.pl\?show=anime&amp;aid=\d+)')
    if anidb_link:
...
def videos_mal(item):
    ...
    if not "No episode video" in data and not "http://www.hulu.com/" in data:
        ...
        if next_page:
            itemlist.append(item.clone(title=">> Más Episodios", url=next_page, text_color=""))
    if itemlist:
        itemlist.insert(0, item.clone(title="Episodios", action="", text_color=color3))
    
    patron = '<a class="iframe.*?href="(https://www.youtube.*?)\?.*?data-src="([^"]+)".*?<span class="title">([^<]+)<'
    matches = scrapertools.find_multiple_matches(data, patron)
    if matches:
        itemlist.append(item.clone(title="Tráilers/Promocionales", action="", text_color=color3))
        for url, thumb, title in matches:
            url = url.replace("embed/", "watch?v=")
            itemlist.append(item.clone(title=title, url=url, server="youtube", action="play", thumbnail=thumb, text_color=color1))
...
def reco_mal(item):
    ...
    data = re.sub(r"\n|\r|\t|&nbsp;", "", data)
    data = re.sub(r"\s{2}", " ", data)
    bloque = scrapertools.find_single_match(data, '<div class="anime-slide-block" id="anime_recommendation"(.*?)</ul></div>')
...
def indices_mal(item):
    ...
    if "Temporadas" in item.title:
        ...
        for url, title in matches:
            ...
            title = title.replace("Winter", "Invierno").replace("Spring", "Primavera") \
                         .replace("Summer", "Verano").replace("Fall", "Otoño")
    ...
    if item.info:
        ...
        if next_page:
            itemlist.append(Item(channel=item.channel, action="season_mal", url=next_page, text_color="",
                                 title=">> Página Siguiente", thumbnail=item.thumbnail))
...
def detail_staff(item):
    ...
    if item.extra == "character" and not "No biography written" in data:
        bio = scrapertools.find_single_match(data, 'itemprop="name">.*?<div class="normal_header".*?</div>(.*?)<div class="normal_header"')
        bio = bio.replace("<br />", "\n")
        ...
        if not "questionmark" in item.thumbnail:
            ...
            for i, thumb in enumerate(matches):
                title = "Imagen %s" % (i+1)
                infoLabels = {'plot': bio}
                itemlist.append(Item(channel=item.channel, action="", title=title, infoLabels=infoLabels, text_color=color1,
                                     thumbnail=thumb))
        
        matches = scrapertools.find_multiple_matches(data, '<a href="(/anime[^"]+)"><img src="([^"]+)".*?href.*?>(.*?)</a>')
        if matches:
...
def musica_anime(item):
    ...
    for number, song, anime, id_anime, id_song in matches:
        if not animes.get(anime):
            animes[anime] = []
            animes[anime].append(Item(channel=item.channel, action=action, title="[COLOR %s][%s][/COLOR]" % (color6, anime.capitalize()), url="",
                                      number="0", thumbnail=item.thumbnail, fanart=item.fanart))
        title = "%s - %s" % (number, song)
        animes[anime].append(Item(channel=item.channel, action="play", title=title, server="directo", url=id_anime, song=id_song, number=number,
                                  thumbnail=item.thumbnail, fanart=item.fanart, text_color=color5))
...
def items_mal(item):
    ...
    for d in data_items:
        ...
        if "Movie" in d["anime_media_type_string"]:
            tipo = "movie"
            extra = "movie"
        itemlist.append(Item(channel=item.channel, action="detalles_mal", url=url, title=title, thumbnail=thumbnail, text_color=color3,
...
def menu_mal(item):
    ...
    try:
        status = {'1': 'Viendo Actualmente', '2': 'Completados', '3': 'En pausa', '4': 'Descartados', '6': 'Previstos para ver'}
        button, estado = scrapertools.find_single_match(data, 'myinfo_updateInfo"(.*?)>.*?option selected="selected" value="(\d+)"')
    ...
    for key, value in status.items():
        if not value in title_estado:
            itemlist.append(item.clone(title="Añadir a lista %s" % value, action="addlist_mal", text_color=color5, value=key, estado=value))
    
    for i in range(10, 0, -1):
        if i != int(score):
            itemlist.append(item.clone(title="Puntuar con un [COLOR %s]%s[/COLOR]" % (color6, i), action="addlist_mal", value=estado, estado=status[estado], score=i))
    return itemlist
...
def play(item):
    ...
    if not item.server:
        ...
        if item.server == "directo" and item.song:
            url = ""
            data_music = jsontools.load_json(httptools.downloadpage("http://www.musicaanime.org/scripts/resources/artists1.php").data)
