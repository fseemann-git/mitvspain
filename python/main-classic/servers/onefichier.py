...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("mitvspain.servers.onefichier get_video_url(page_url='%s')" % page_url)
    ...
    if config.get_setting("onefichierpremium")=="true":
        ...
        logger.info("mitvspain.servers.onefichier url="+url)
        post_parameters = {"mail":user,"pass":password,"lt":"on","purge":"on","valider":"Send"}
        post = urllib.urlencode(post_parameters)
        logger.info("mitvspain.servers.onefichier post="+post)

        data = scrapertools.cache_page(url, post=post)
        #logger.info("mitvspain.servers.onefichier data="+data)

        cookies = config.get_cookie_data()
        logger.info("mitvspain.servers.onefichier cookies="+cookies)
        ...
        logger.info("mitvspain.servers.onefichier sid_cookie_value="+sid_cookie_value)
        ...
        logger.info("mitvspain.servers.onefichier filename="+filename)

        # Construye la URL final para Kodi
        location = page_url+"|Cookie="+cookie
        logger.info("mitvspain.servers.onefichier location="+location)

        video_urls = []
        video_urls.append( [filename[-4:]+" (Premium) [1fichier]" , location] )

    for video_url in video_urls:
        logger.info("mitvspain.servers.onefichier %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(data):
    ...
    logger.info("mitvspain.servers.onefichier find_videos #"+patronvideos+"#")
